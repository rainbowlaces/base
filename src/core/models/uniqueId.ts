...
export class UniqueID {
    ...
    private validateAndNormalize(id: string): string {
        // Check length
        if (id.length !== 20) {
            throw new Error(`Invalid UniqueID length: expected 20, got ${id.length}`);
        }

        // Check that all characters are valid (0-9, a-z)
        if (!/^[0-9a-z]+$/.test(id)) {
            throw new Error('Invalid UniqueID format: contains invalid characters');
        }
        ...
        if (isNaN(timestamp)) {
            throw new Error('Invalid UniqueID: timestamp part is not a valid base36 number');
        }
        ...
        if (timestamp < year2000 || timestamp > futureLimit) {
            throw new Error(`Invalid UniqueID: timestamp ${date.toISOString()} is outside reasonable range`);
        }
