    ...
    for (const phaseNumber of sortedPhaseNumbers) {
      ...
      const phasePromises = itemsInPhase.map(async (item) => {
        ...
        try {
          ...
          if (!registration) {
            throw new Error(`No registration found for '${item.name}'`);
          }
          
          if (!registration.singleton) {
            throw new Error(`Cannot run setup on non-singleton service '${item.name}'. Setup/teardown only works with singleton: true services.`);
          }
