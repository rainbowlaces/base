...
import { type BaseAction, type ContextState, type ModuleStatusArgs, type TopicFunction } from "./types";
...
> extends EventEmitter {
  ...
  protected async coordinateAndRun(topic: string): Promise<void> {
    ...
    if (this.#phaseMap.size === 0) {
      this.logger.debug(`No handlers found for topic: ${topic} - fast failing`);
